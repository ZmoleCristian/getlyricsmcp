...
#   5. AUR push                 → getlyricsmcp + getlyricsmcp-bin
#
...
#   ./release.sh 0.1.1       # asserts Cargo.toml matches argument
...
# Untracked files are deliberate here (CLAUDE.md, law.toml, .claude/, .mcp.json are
# kept out of the repo without being gitignored), so only tracked changes count as dirty.
if [ -n "$(git status --porcelain --untracked-files=no)" ]; then
  echo "FATAL: tracked files are modified. Commit or stash before releasing." >&2
...
if cargo search getlyricsmcp --limit 1 2>/dev/null | grep -qE "^getlyricsmcp = \"$VERSION\""; then
  echo "  getlyricsmcp $VERSION already on crates.io, skipping publish"
else
...
push_aur() {
  ...
  [ -f "$src_dir/getlyricsmcp.install" ] && cp "$src_dir/getlyricsmcp.install" "$workdir/getlyricsmcp.install"
  ...
  (
    ...
    [ -f getlyricsmcp.install ] && git add getlyricsmcp.install
...
echo "Pushing to AUR..."
push_aur getlyricsmcp
push_aur getlyricsmcp-bin

echo
echo "============================================================"
echo "  release $TAG fully shipped:"
echo "    GH release v$VERSION + binaries"
echo "    crates.io: getlyricsmcp $VERSION"
echo "    AUR: getlyricsmcp + getlyricsmcp-bin"
